...
function ProjectContent({title,subtitle,images,icon,github,appstore,playstore,producthuntId,producthunt,video,isIpad,titleUrl,techUsed}){          
    ...
    if(appstore){
        appstoreLink = <a href={appstore} target="_blank" rel="noreferrer"><Image  src="/appstorebutton.svg" alt="download on app store" sx={buttonStyle}  /></a>
    }
    if(github){
        githubLink = <a href={github} target="_blank" rel="noreferrer"><Image src="/githubbutton.png" alt="view on github" sx={buttonStyle}  /> </a>
    }
    if(playstore){
        playstoreLink = <a href={playstore} target="_blank" rel="noreferrer"><Image  src="/playstorebutton.svg" alt="download on playstore store" sx={buttonStyle}  /></a>
    }
    if(producthunt && producthuntId){
        producthuntLink = <div><p sx={{height:30,width:50}}/><a href={producthunt} target="_blank" rel="noreferrer"><Image src={"https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id="+producthuntId+"&theme=dark"} alt="Producthunt button" /></a></div>
    }
    return (
